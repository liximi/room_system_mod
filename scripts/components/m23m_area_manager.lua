...
function AreaManager:RefreshClientData(area, remove)	--Area Instance
	...
	for _, player in ipairs(AllPlayers) do
		if remove then
			SendModRPCToClient(CLIENT_MOD_RPC[M23M.RPC_NAMESPACE].remove_area, player.userid, area:GetID())
		else
			...
			SendModRPCToClient(CLIENT_MOD_RPC[M23M.RPC_NAMESPACE].refresh_area_data, player.userid, area:GetID(), json.encode(_tiles), area:GetType())
		end
	end
end
